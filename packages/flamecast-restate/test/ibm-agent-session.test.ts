 ...
 *
 * Mock ACP agent as HTTP server. Test create session → runAgent → verify state.
 */
...
async function startMockAgent(): Promise<MockAgent> {
  const server = http.createServer(async (req, res) => {
    ...
    // GET /agents/echo — agent discovery
    if (req.method === "GET" && url.includes("/agents/echo")) {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ name: "echo", description: "test echo agent" }));
      return;
    }

    // POST /runs — create run (async mode returns immediately)
    if (req.method === "POST" && url === "/runs") {
      const runId = "run-" + Date.now();
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ run_id: runId, status: "running" }));
      return;
    }
...
describe("IbmAgentSession VO", () => {
  ...
  it("runAgent creates run and suspends on awakeable", async () => {
    ...
    expect(pendingRun!.runId).toMatch(/^run-/);
    ...
    await ingress.resolveAwakeable(pendingRun!.awakeableId, {
      status: "completed",
      output: [{ role: "assistant", parts: [{ contentType: "text/plain", content: "Echo: hello" }] }],
    });
